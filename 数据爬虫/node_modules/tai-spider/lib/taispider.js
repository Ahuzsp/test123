...
class TaiSpider extends EventEmitter {
  ...
  */
	...
	setLimiterProperty(limiter, property, value) {
		...
		switch (property) {
			case 'rateLimit':
				self.limiters.key(limiter).setRateLimit(value);break;
			default:
				break;
		}
	}
	...
	processRequest(item, self) {
		...
		setTimeout(() => {
			if (direct) self.direct(curr);else self.queue([curr]);
		}, 100);
	}
			...
			callback: function (err, res) {
				try {
					...
					if (err) {
						...
						self.clearErrors();
						if (item.cb) return item.cb.bind(self)(res, self);else return self.parse.bind(self)(res, self);
					}
				} catch (error) {
					self.errorHandle(error);
				}
