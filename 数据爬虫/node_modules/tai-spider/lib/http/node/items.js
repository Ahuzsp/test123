...
class ItemNode {
	...
	next(selector) {
		...
		if (nodes && nodes.length > 0) return new ItemNode(nodes[0], this.$);
		return null;
	}

	child(nth) {
		let nodes = this.$(this.node).children();
		if (nodes && nodes.length > nth) return new ItemNode(nodes[nth], this.$);
		return null;
	}
...
class ItemNodes {
	...
	next() {
		if (this.current < this.nodes.length) return { done: false, value: this.nodes[this.current++] };
