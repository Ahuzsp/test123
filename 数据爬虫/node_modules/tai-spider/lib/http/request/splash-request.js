...
class SplashRequest {
	...
	fetch(spider, options) {
		...
		if (!splashServer) throw new Error('Please set SPLASH_SERVER first!');
		...
		var doRequest = function (err) {
			if (err) {
				...
				switch (err.op) {
					case 'retry':
						log.debug(err.message + ', retry ' + options.uri);self.onContent(err, options);break;
					case 'fail':
						log.debug(err.message + ', fail ' + options.uri);options.callback(err, { options: options }, options.release);break;
					case 'abort':
						log.debug(err.message + ', abort ' + options.uri);options.release();break;
					case 'queue':
						log.debug(err.message + ', queue ' + options.uri);self.queue(options);options.release();break;
					default:
						log.debug(err.message + ', retry ' + options.uri);self.onContent(err, options);break;
				}
				return;
