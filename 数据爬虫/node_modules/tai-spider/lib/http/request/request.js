	...
	fetch(spider, options) {
		...
		var doRequest = function (err) {
			if (err) {
				...
				switch (err.op) {
					case 'retry':
						log.debug(err.message + ', retry ' + options.uri);spider.onContent(err, options);break;
					case 'fail':
					...
					case 'queue':
						log.debug(err.message + ', queue ' + options.uri);spider.queue(options);options.release();break;
					default:
						log.debug(err.message + ', retry ' + options.uri);spider.onContent(err, options);break;
				}
				return;
