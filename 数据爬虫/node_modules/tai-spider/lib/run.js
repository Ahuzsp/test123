...
const runSpider = spider => {
	...
	spider.on('_init_finished', function () {
		...
		let start_urls = [];
		if (typeof spider.start_urls === 'function') start_urls = spider.start_urls();else start_urls = spider.start_urls;
		start_urls.forEach(url => {
			if (typeof url === 'string') {
				debug('start url: %s', url);
				const { headers = {} } = spider;
				spider.processRequest(new Request({
					link: url,
					headers,
...
pkgs.forEach(function (pkg) {
	if (pkg in spiders) {
		...
		runSpider(spider);
	} else error('Not found spider ', pkg);
});